...
import PhotoWallPhoto from './photo-wall-photo.js';

const PhotoWall = ({ imagesArray, padding = 0, columns = [1, 2, 3] }) => {
  const imageNodes = imagesArray.map((arr, index) => {
    return (
      <PhotoWallPhoto key={index} url={arr.url} columns={columns} padding={padding} clickHandler={arr.clickHandler} />
    );
  });
